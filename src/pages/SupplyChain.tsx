...
export default function SupplyChain() {
  ...
  const sankeyOption = {
    ...
    series: [{
      ...
      layoutIterations: 0,
      data: [
        { name: 'Tier 3\n原材料', itemStyle: { color: '#f59e0b' } },
        { name: 'Tier 3\n物流仓储', itemStyle: { color: '#f59e0b' } },
        { name: 'Tier 2\n面料染整', itemStyle: { color: '#3b82f6' } },
        { name: 'Tier 1\n成衣制造', itemStyle: { color: '#1a6b47' } },
        { name: 'ABC Fashion', itemStyle: { color: '#7c3aed' } },
      ],
      links: [
        { source: 'Tier 3\n原材料', target: 'Tier 2\n面料染整', value: 3 },
        { source: 'Tier 3\n物流仓储', target: 'Tier 1\n成衣制造', value: 1 },
  ...
  const tierPieOption = {
    ...
    series: [{
      ...
      data: [
        { value: tier1.length, name: 'Tier 1 成衣制造', itemStyle: { color: '#1a6b47' } },
        { value: tier2.length, name: 'Tier 2 面料染整', itemStyle: { color: '#3b82f6' } },
        { value: tier3.length, name: 'Tier 3 原材料/物流', itemStyle: { color: '#f59e0b' } },
      ],
    }],
  };
  ...
  const barOption = {
    ...
    legend: { data: ['碳排放(tCO₂e)', '水耗(百m³)'], bottom: 0 },
    grid: { left: '3%', right: '4%', bottom: '15%', top: '10%', containLabel: true },
    xAxis: { type: 'category', data: ['Tier 1', 'Tier 2', 'Tier 3'], axisLabel: { fontSize: 12 } },
    yAxis: [
    ...
    series: [
      {
        ...
        itemStyle: { color: '#ef4444' }, barWidth: '35%',
      },
      {
        ...
        itemStyle: { color: '#3b82f6' }, barWidth: '35%',
      },
    ],
  };
      ...
      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-card-info">
            ...
            <div className="stat-card-change">高风险 {tier1.filter(s => s.riskLevel === 'high').length} 家</div>
          </div>
        ...
        <div className="stat-card">
          <div className="stat-card-info">
            ...
            <div className="stat-card-value">
              {(suppliers.reduce((a, b) => a + b.carbonScope1 + b.carbonScope2 + b.carbonScope3, 0) / 1000).toFixed(0)}k
            </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>ESG评级</th><th>风险等级</th><th>碳排放(tCO₂e)</th><th>水耗(m³)</th>
                  <th>审计得分</th><th>认证数</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(s => (
                  <tr key={s.id}>
                    ...
                    <td>{s.location}</td>
                    <td>
                      <span className={`badge ${s.esgRating === 'A' ? 'badge-green' : s.esgRating === 'B' ? 'badge-blue' : s.esgRating === 'C' ? 'badge-yellow' : 'badge-red'}`}>
                        {s.esgRating}
                      </span>
                    </td>
